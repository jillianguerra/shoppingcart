...
describe('Test the cart endpoints', () => {
    ...
    test('It should change item quantities and delete an item', async () => {
        const user = new User({ name: 'Meriadoc Brandybuck', email: 'merrybrandy@email.com', password: 'secondbreakfast'})
        await user.save()
        const token = await user.generateAuthToken()
        const cart = new Cart({ user: user._id })
        await cart.save()
        const item1 = await new Item({ name: 'pots and pans', description: 'good for cooking and throwing', category: 'cookware', price: 100 })
        await item1.save()
        const item2 = await new Item({ name: 'crispy bacon', description: 'delicious', category: 'meats', price: 10})
        const itemList1 = new ItemList({ item: item1._id, quantity: 1, cart: cart._id })
        await itemList1.save()
        const itemList2 = new ItemList({ item: item2._id, quantity: 6, cart: cart._id })
        await itemList2.save()
        cart.items = ([itemList1, itemList2])
        await cart.save()
        const response = await request(app)
            ...
            .send()
        expect(response.statusCode).toBe(200)
        expect(response.body.total).toEqual(30)
    })
})