...
describe('Test the item endpoints', () => {
    ...
    test('It should update an item', async () => {
        const item = new Item({ name: 'lamp', description: 'stop sitting in the dark and get a lamp!', price: 40 })
        await item.save()
        const response = await request(app)
            .put(`/items/${item._id}`)
            .send({ name: 'fancy lamp', description: `It's like a lamp but so much fancier!`, price: 200 })
    ...
    test('It should display an item', async () => {
        const item = new Item({ name: 'fancy lamp', description: `It's like a lamp but so much fancier!`, price: 200 })
        await item.save()
        const response = await request(app)
            .get(`/items/${item._id}`)
    expect(response.body.name).toEqual('fancy lamp')
    ...
    test('It should display all items', async() => {
        const item1 = new Item({ name: 'lamp', description: 'stop sitting in the dark and get a lamp!', price: 40 })
        await item1.save()
        const item2 = new Item({ name: 'fancy lamp', description: `It's like a lamp but so much fancier!`, price: 200 })
        await item2.save()
        const response = await request(app)
            .get('/items')
        expect(response.statusCode).toBe(200)
